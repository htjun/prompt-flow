...
import { BUTTON_DIMENSIONS } from '@/constants/flowConstants'
...
export const calculateHandleOffset = (actions: string[], targetIndex: number): CSSProperties => {
  ...
  const buttonWidths = actions.map((action) => {
    const textWidth = action.length * BUTTON_DIMENSIONS.CHAR_WIDTH
    return textWidth + BUTTON_DIMENSIONS.PADDING_X * 2 // Add left and right padding
  })

  // Calculate position from right edge
  let rightOffset = BUTTON_DIMENSIONS.CONTAINER_PADDING

  // Add widths of buttons to the right of target
  for (let i = actions.length - 1; i > targetIndex; i--) {
    rightOffset += buttonWidths[i]
    if (i > targetIndex + 1) rightOffset += BUTTON_DIMENSIONS.GAP
  }
